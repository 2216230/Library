        ...
        <div class="box-header with-border" style="background: linear-gradient(135deg, #f0fff0 0%, #e0f7e0 100%); padding: 20px; border-bottom: 2px solid #20650A;">
          <div class="row">
            ...
            <div class="col-md-6 text-right">
              <a href="subjects.php" class="btn btn-success btn-flat" style="background: linear-gradient(135deg, #32CD32 0%, #184d08 100%); color: white; border: none; border-radius: 6px; font-weight: 600; padding: 8px 20px;">
          ...
          <div class="table-responsive">
            <table class="table table-striped table-hover" style="border-radius: 8px; overflow: hidden; font-size: 14px; margin-bottom: 0;">
              <thead style="background: linear-gradient(135deg, #20650A 0%, #184d08 100%); color: white; font-weight: 700;">
                <tr>
                  <th style="color: white; padding: 13px 8px; font-weight: 700; width: 60px;">#</th>
                  <th style="color: white; padding: 13px 8px; font-weight: 700;">Subject Name</th>
                  <th style="color: white; padding: 13px 8px; font-weight: 700;">Date Archived</th>
              ...
              <tbody>
                <?php
                  ...
                  if($query->num_rows > 0){
                    ...
                    while($row = $query->fetch_assoc()){
                      echo "
                        <tr>
                          ...
                          <td style='padding: 12px 8px;'>".htmlspecialchars($row['date_archived'] ?? '')."</td>
                          <td style='padding: 12px 8px;'>
                            <div class='btn-group btn-group-sm' role='group'>
                              <form method='POST' action='subject_restore.php' style='display:inline-block;'>
                                <input type='hidden' name='id' value='".$row['id']."'>
                                <button class='btn btn-warning' type='submit' title='Restore subject' style='font-size: 12px; padding: 5px 10px;'>
                                  <i class='fa fa-undo'></i> Restore
                                </button>
                              </form>
                              <form method='POST' action='subject_delete_permanently.php' style='display:inline-block; margin-left: 3px;' onsubmit=\"return confirm('Are you sure you want to permanently delete this subject?');\">
                                <input type='hidden' name='id' value='".$row['id']."'>
                    ...
                    echo "<tr><td colspan='4' class='text-center' style='padding: 30px; color: #999;'><i class='fa fa-inbox' style='font-size: 24px; margin-right: 10px;'></i>No archived subjects found</td></tr>";
                  }
                ?>
              </tbody>
            </table>
