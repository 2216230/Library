...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    header('Location: overdue_management.php?tab=lost');
    exit;
}
...
if ($adminId) {
    $action = 'FOUND';
    $description = "Lost book found and returned: {$book_title} | Borrower: {$borrower_name} | Status changed: Lost → Returned";
    logActivity($conn, $adminId, $action, $description, 'borrow_transactions', $id);
}

// Redirect back to lost tab in overdue management
header('Location: overdue_management.php?tab=lost');
exit;
?>